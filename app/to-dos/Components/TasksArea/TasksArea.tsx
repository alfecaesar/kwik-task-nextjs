...
export function SingleTask({ singleTask }: { singleTask: Task }) {
  ...
  const lowerOpacity = singleTask.status === "completed" && "opacity-45";

  const lineThrough = singleTask.status === "completed" && "line-through";
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-row md:flex-col gap-1">
          ...
          <Badge variant="outline" className="text-[10px] opacity-55 w-fit">
            {singleTask.status}
          </Badge>
        </div>
      </div>
